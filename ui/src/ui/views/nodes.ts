      ...
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            ${t('nodes.approveButton')}
          </button>
          <button class="btn btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            ${t('nodes.rejectButton')}
          </button>
        </div>
      </div>
    </div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t('nodes.revokedStatus') : t('nodes.activeStatus');
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
          <div class="card-title">${t('nodes.execNodeBindingTitle')}</div>
          <div class="card-sub">${t('nodes.execNodeBindingSub')}</div>
        </div>
        <button
          class="btn"
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t('agents.saving') : t('common.save')}
        </button>
      </div>
        ...
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t('common.loading') : t('nodes.loadConfigButton')}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t('nodes.pairedChip') : t('nodes.unpairedChip')}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t('nodes.connectedChip') : t('nodes.offlineChip')}
          </span>
