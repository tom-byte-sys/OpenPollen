...
export function renderNode(params: {
  ...
  if (unsupported.has(key)) {
    return html`<div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">${t('config.unsupportedSchema')}</div>
    </div>`;
  }
  ...
  const placeholder =
    ...
    (isSensitive
      ...
      : schema.default !== undefined
        ? t('config.defaultPrefix', { value: String(schema.default) })
        : "");
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t('config.selectOption')}</option>
        ${options.map(
          (opt, idx) => html`
...
function renderArray(params: {
  ...
  if (!itemsSchema) {
    return html`
      <div class="cfg-field cfg-field--error">
        <div class="cfg-field__label">${label}</div>
        <div class="cfg-field__error">${t('config.unsupportedArray')}</div>
      </div>
    `;
  }
  ...
  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">${t('config.itemCount', { count: arr.length })}</span>
        <button
          type="button"
      ...
      ${
        arr.length === 0
          ? html`
              <div class="cfg-array__empty">${t('config.noItems')}</div>
            `
          : html`
            ...
            <div class="cfg-array__item">
              <div class="cfg-array__item-header">
                ...
                <button
                  ...
                  title=${t('config.removeItemTitle')}
                  ?disabled=${disabled}
                  @click=${() => {
                    const next = [...arr];
                    next.splice(idx, 1);
