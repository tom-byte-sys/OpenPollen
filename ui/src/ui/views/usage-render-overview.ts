  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">${t('usage.daily')}</div>
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">${t('usage.daily')} ${isTokenMode ? t('usage.tokens') : t('usage.cost')}</div>
      </div>
    ...
    <div class="cost-breakdown cost-breakdown-compact">
      ...
      <div class="cost-breakdown-bar">
        <div class="cost-segment output" style="width: ${(isTokenMode ? tokenPcts.output : breakdown.output.pct).toFixed(1)}%"
          title="${t('usage.outputLabel')}: ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}"></div>
        <div class="cost-segment input" style="width: ${(isTokenMode ? tokenPcts.input : breakdown.input.pct).toFixed(1)}%"
          title="${t('usage.inputLabel')}: ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}"></div>
        <div class="cost-segment cache-write" style="width: ${(isTokenMode ? tokenPcts.cacheWrite : breakdown.cacheWrite.pct).toFixed(1)}%"
          title="${t('usage.cacheWriteLabel')}: ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}"></div>
        <div class="cost-segment cache-read" style="width: ${(isTokenMode ? tokenPcts.cacheRead : breakdown.cacheRead.pct).toFixed(1)}%"
          title="${t('usage.cacheReadLabel')}: ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}"></div>
      </div>
      <div class="cost-breakdown-legend">
        <span class="legend-item"><span class="legend-dot output"></span>${t('usage.outputLabel')} ${isTokenMode ? formatTokens(totals.output) : formatCost(breakdown.output.cost)}</span>
        <span class="legend-item"><span class="legend-dot input"></span>${t('usage.inputLabel')} ${isTokenMode ? formatTokens(totals.input) : formatCost(breakdown.input.cost)}</span>
        <span class="legend-item"><span class="legend-dot cache-write"></span>${t('usage.cacheWriteLabel')} ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}</span>
        <span class="legend-item"><span class="legend-dot cache-read"></span>${t('usage.cacheReadLabel')} ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}</span>
      </div>
      ...
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${t('usage.userMessages', { count: aggregates.messages.user })} · ${t('usage.assistantMessages', { count: aggregates.messages.assistant })}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t('usage.toolCallsTitle')}
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ...
            <span class="usage-summary-hint" title=${t('usage.tokensHintText')}>?</span>
          </div>
          <div class="usage-summary-value">${formatTokens(avgTokens)}</div>
          <div class="usage-summary-sub">${t('usage.acrossMessages', { count: aggregates.messages.total || 0 })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
          ...
          <div class="usage-summary-sub">${formatCost(totals.totalCost)} ${t('usage.totalSuffix')}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t('usage.sessionsTitle')}
            <span class="usage-summary-hint" title=${t('usage.sessionsHintTitle')}>?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">${t('usage.ofSessionsInRange', { count: totalSessions })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t('usage.throughputTitle')}
            <span class="usage-summary-hint" title=${t('usage.throughputHintText')}>?</span>
          </div>
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t('usage.errorRateTitle')}
            <span class="usage-summary-hint" title=${t('usage.errorRateHint')}>?</span>
          </div>
          <div class="usage-summary-value ${errorRatePct > 5 ? "bad" : errorRatePct > 1 ? "warn" : "good"}">${errorRatePct.toFixed(2)}%</div>
          <div class="usage-summary-sub">
            ${aggregates.messages.errors} ${t('usage.errorsLower')} · ${avgDurationLabel} ${t('usage.avgSession')}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t('usage.cacheHitRateTitle')}
            <span class="usage-summary-hint" title=${t('usage.cacheHitHint')}>?</span>
          </div>
    ...
    <div class="card sessions-card">
      ...
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          ...
          <span>${totalErrors} ${t('usage.errorsLower')}</span>
        </div>
        ...
        <label class="sessions-sort">
          <span>${t('usage.sortLabel')}</span>
          <select
            @change=${(e: Event) => onSessionSortChange((e.target as HTMLSelectElement).value as typeof sessionSort)}
          >
            <option value="cost" ?selected=${sessionSort === "cost"}>${t('usage.costOption')}</option>
            <option value="errors" ?selected=${sessionSort === "errors"}>${t('usage.errorsOption')}</option>
            <option value="messages" ?selected=${sessionSort === "messages"}>${t('usage.messagesOption')}</option>
            <option value="recent" ?selected=${sessionSort === "recent"}>${t('usage.recentOption')}</option>
            <option value="tokens" ?selected=${sessionSort === "tokens"}>${t('usage.tokensOption')}</option>
          </select>
        </label>
        <button
          class="btn btn-sm sessions-action-btn icon"
          @click=${() => onSessionSortDirChange(sessionSortDir === "desc" ? "asc" : "desc")}
          title=${sessionSortDir === "desc" ? t('usage.descendingTitle') : t('usage.ascendingTitle')}
        >
          ${sessionSortDir === "desc" ? "↓" : "↑"}
        </button>
        ${
          ...
          : sessions.length === 0
            ...
            : html`
	                <div class="session-bars">
	                  ...
	                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">+${sessions.length - 50} ${t('usage.moreLabel')}</div>` : nothing}
	                </div>
	              `
      }
      ${
