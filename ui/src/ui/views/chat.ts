        ...
        props.queue.length
          ? html`
            <div class="chat-queue" role="status" aria-live="polite">
              <div class="chat-queue__title">${t('chat.queuedTitle')} (${props.queue.length})</div>
                  ...
                  (item) => html`
                    <div class="chat-queue__item">
                      <div class="chat-queue__text">
                        ${
                          item.text ||
                          (item.attachments?.length ? t('chat.imageCount', { count: item.attachments.length }) : "")
                        }
                      </div>
                      <button
...
function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
  ...
  if (historyStart > 0) {
    items.push({
      ...
      message: {
        role: "system",
        content: t('chat.showingLast', { count: CHAT_HISTORY_RENDER_LIMIT, hidden: historyStart }),
        timestamp: Date.now(),
      },
    });
  }
  for (let i = historyStart; i < history.length; i++) {
    ...
    const marker = (raw.__openpollen ?? raw.__openclaw) as Record<string, unknown> | undefined;
