...
import { formatDurationCompact } from "../../shared/format-time/format-duration.ts";
...
function renderSessionSummary(session: UsageSessionEntry) {
  ...
  if (!usage) {
    return html`
      <div class="muted">${t('usage.noDataForSession')}</div>
    `;
  }
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        <div class="session-summary-title">${t('usage.messagesTitle')}</div>
        <div class="session-summary-value">${usage.messageCounts?.total ?? 0}</div>
        <div class="session-summary-meta">${t('usage.userMessages', { count: usage.messageCounts?.user ?? 0 })} · ${t('usage.assistantMessages', { count: usage.messageCounts?.assistant ?? 0 })}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t('usage.toolCallsTitle')}</div>
      ...
      <div class="session-summary-card">
        <div class="session-summary-title">${t('usage.errorsTitle')}</div>
      ...
      <div class="session-summary-card">
        <div class="session-summary-title">${t('usage.durationTitle')}</div>
      ...
      <div class="session-detail-header">
        ...
        <div class="session-detail-stats">
          ${
            usage
              ? html`
            <span><strong>${formatTokens(usage.totalTokens)}</strong> ${t('usage.tokensOption')}</span>
            <span><strong>${formatCost(usage.totalCost)}</strong></span>
          `
              : nothing
          }
        </div>
        <button class="session-close-btn" @click=${onClose} title=${t('usage.closeDetails')}>×</button>
      </div>
...
function renderTimeSeriesCompact(
  ...
  return html`
    <div class="session-timeseries-compact">
      ...
      <svg viewBox="0 0 ${width} ${height + 15}" class="timeseries-svg" style="width: 100%; height: auto;">
        ...
        ${points.map((p, i) => {
          ...
          const tooltipLines = [
            ...
            `${formatTokens(val)} ${t('usage.tokensSuffix')}`,
          ];
          if (breakdownByType) {
            tooltipLines.push(t('usage.outputBreakdown', { value: formatTokens(p.output) }));
            tooltipLines.push(t('usage.inputBreakdown', { value: formatTokens(p.input) }));
            tooltipLines.push(t('usage.cacheWriteBreakdown', { value: formatTokens(p.cacheWrite) }));
            tooltipLines.push(t('usage.cacheReadBreakdown', { value: formatTokens(p.cacheRead) }));
          }
      ...
      <div class="timeseries-summary">${t('usage.msgsCount', { count: String(points.length) })} · ${formatTokens(cumTokens)} · ${formatCost(cumCost)}</div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                ...
                <div class="cost-breakdown-legend">
                  <div class="legend-item" title=${t('usage.outputTokensHint')}>
                    <span class="legend-dot output"></span>${t('usage.outputLabel')} ${formatTokens(sumOutput)}
                  </div>
                  <div class="legend-item" title=${t('usage.inputTokensHint')}>
                    <span class="legend-dot input"></span>${t('usage.inputLabel')} ${formatTokens(sumInput)}
                  </div>
                  <div class="legend-item" title=${t('usage.cacheWriteHint')}>
                    <span class="legend-dot cache-write"></span>${t('usage.cacheWriteLabel')} ${formatTokens(sumCacheWrite)}
                  </div>
                  <div class="legend-item" title=${t('usage.cacheReadHint')}>
                    <span class="legend-dot cache-read"></span>${t('usage.cacheReadLabel')} ${formatTokens(sumCacheRead)}
                  </div>
                </div>
                <div class="cost-breakdown-total">${t('usage.totalColon')} ${formatTokens(totalTypeTokens)}</div>
              </div>
            `
          : nothing
      }
...
function renderContextPanel(
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t('usage.ofInput', { pct: Math.min((totalContextTokens / inputTokens) * 100, 100).toFixed(0) });
    }
  }
        ...
        ${
          hasMore
            ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t('usage.collapseLogs') : t('usage.expandAll')}
              </button>`
            : nothing
        }
      </div>
      <p class="context-weight-desc">${contextPct || t('usage.baseContext')}</p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title="${t('usage.systemContext', { value: formatTokens(systemTokens) })}"></div>
        <div class="context-segment skills" style="width: ${pct(skillsTokens, totalContextTokens).toFixed(1)}%" title="${t('usage.skillsContext', { value: formatTokens(skillsTokens) })}"></div>
        <div class="context-segment tools" style="width: ${pct(toolsTokens, totalContextTokens).toFixed(1)}%" title="${t('usage.toolsContext', { value: formatTokens(toolsTokens) })}"></div>
        <div class="context-segment files" style="width: ${pct(filesTokens, totalContextTokens).toFixed(1)}%" title="${t('usage.filesContext', { value: formatTokens(filesTokens) })}"></div>
      </div>
      <div class="context-legend">
        <span class="legend-item"><span class="legend-dot system"></span>${t('usage.sysPrefix')} ~${formatTokens(systemTokens)}</span>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t('usage.skillsLabel')} (${skillsList.length})</div>
        ...
        ${
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t('usage.toolsLabel')} (${toolsList.length})</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
                          <div class="context-breakdown-item">
        ...
        ${
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t('usage.filesLabel')} (${filesList.length})</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  const displayedCount =
    filters.roles.length > 0 || filters.tools.length > 0 || filters.hasTools || normalizedQuery
      ? t('usage.filteredOfTotal', { filtered: String(filteredEntries.length), total: String(logs.length) })
      : `${logs.length}`;
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t('usage.conversation')} <span style="font-weight: normal; color: var(--text-muted);">(${displayedCount})</span></span>
      ...
      <div class="session-logs-list">
        ${filteredEntries.map((entry) => {
          ...
          const roleLabel =
            log.role === "user" ? t('usage.youLabel') : log.role === "assistant" ? t('usage.assistantRole') : t('usage.toolRole');
          return html`
