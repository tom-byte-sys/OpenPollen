      ...
      <section class="card agents-sidebar">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="card-sub">${t('agents.configuredCount', { count: agents.length })}</div>
          </div>
...
function renderAgentOverview(params: {
  ...
  const identityStatus = agentIdentityLoading
    ? t('agents.identityLoadingStatus')
    : agentIdentityError
      ? t('agents.identityUnavailable')
      : "";
  const isDefault = Boolean(params.defaultId && agent.id === params.defaultId);

  return html`
    <section class="card">
      <div class="card-title">${t('agents.panel.overview')}</div>
      ...
      <div class="agents-overview-grid" style="margin-top: 16px;">
        ...
        <div class="agent-kv">
          ...
          <div>${skillFilter ? t('agents.skillsSelected', { count: skillCount ?? 0 }) : t('agents.allSkills')}</div>
        </div>
      </div>

      <div class="agent-model-select" style="margin-top: 20px;">
        <div class="label">${t('agents.modelSelectionTitle')}</div>
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          <label class="field" style="min-width: 260px; flex: 1;">
            <span>${t('agents.primaryModelField')}${isDefault ? ` (${t('agents.default')})` : ""}</span>
            <select
            ...
            >
              ${
                isDefault
                  ...
                  : html`
                      <option value="">
                        ${defaultPrimary ? t('agents.inheritDefaultWith', { model: defaultPrimary }) : t('agents.inheritDefaultSimple')}
                      </option>
                    `
              }
