  ...
  return html`
    <section class="card">
      ...
      <div class="agents-overview-grid" style="margin-top: 16px;">
        <div class="agent-kv">
          <div class="label">${t('agents.workspaceLabel')}</div>
          <div class="mono">${context.workspace}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t('agents.primaryModelLabel')}</div>
          <div class="mono">${context.model}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t('agents.identityNameLabel')}</div>
          <div>${context.identityName}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t('agents.identityEmojiLabel')}</div>
          <div>${context.identityEmoji}</div>
        </div>
        <div class="agent-kv">
          <div class="label">${t('agents.skillsFilterLabel')}</div>
...
export function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t('agents.contextIdentitySub'))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t('agents.channelsPanelTitle')}</div>
            <div class="card-sub">${t('agents.channelsPanelSub')}</div>
          </div>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t('agents.refreshingLabel') : t('common.refresh')}
          </button>
        </div>
        <div class="muted" style="margin-top: 8px;">
          ${t('agents.lastRefreshPrefix')} ${lastSuccessLabel}
        </div>
        ${
          params.error
        ...
        ${
          !params.snapshot
            ? html`
                <div class="callout info" style="margin-top: 12px">${t('agents.loadChannelsHint')}</div>
              `
            : nothing
        }
        ${
          entries.length === 0
            ? html`
                <div class="muted" style="margin-top: 16px">${t('agents.noChannelsFoundPanel')}</div>
              `
            : html`
...
export function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t('agents.contextSchedulingSub'))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
...
export function renderAgentFiles(params: {
  ...
  return html`
    <section class="card">
      ...
      ${
        list
          ? html`<div class="muted mono" style="margin-top: 8px;">${t('agents.workspacePrefix')} ${list.workspace}</div>`
          : nothing
      }
      ${
                      ...
                      : html`
                          <div class="agent-file-header">
                            ...
                            <div class="agent-file-actions">
                              ...
                              >
                                ${params.agentFileSaving ? t('agents.saving') : t('common.save')}
                              </button>
                            </div>
                          </div>
                          ${
                          ...
                          <label class="field" style="margin-top: 12px;">
                            <span>${t('agents.contentLabel')}</span>
                            <textarea
                              .value=${draft}
                              @input=${(e: Event) =>
