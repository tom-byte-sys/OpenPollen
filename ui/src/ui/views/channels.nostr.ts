    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t('channels.nostr.profileLabel')}</div>
          ${
            summaryConfigured
              ? html`
                <button
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                ...
                ${name ? html`<div><span class="label">${t('channels.nostr.nameLabel')}</span><span>${name}</span></div>` : nothing}
                ${
                  displayName
                    ? html`<div><span class="label">${t('channels.nostr.displayNameLabel')}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
                    ? html`<div><span class="label">${t('channels.nostr.aboutLabel')}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                    : nothing
                }
                ${nip05 ? html`<div><span class="label">${t('channels.nostr.nip05Label')}</span><span>${nip05}</span></div>` : nothing}
              </div>
            `
            : html`
