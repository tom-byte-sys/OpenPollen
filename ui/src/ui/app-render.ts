...
import { parseAgentSessionKey } from "../shared/sessions/session-key.js";
import { refreshChatAvatar } from "./app-chat.ts";
import { renderUsageTab } from "./app-render-usage-tab.ts";
import { renderChatControls, renderLocaleToggle, renderTab, renderThemeToggle, titleKeyForTab } from "./app-render.helpers.ts";
...
const NAV_GROUP_KEYS: Record<string, TranslationKey> = {
  ...
  Agent: 'nav.group.agent',
  Settings: 'nav.group.settings',
};

const TAB_SUBTITLE_KEYS: Record<string, TranslationKey> = {
  agents: 'nav.sub.agents',
  overview: 'nav.sub.overview',
  channels: 'nav.sub.channels',
  instances: 'nav.sub.instances',
  sessions: 'nav.sub.sessions',
  usage: 'nav.sub.usage',
  cron: 'nav.sub.cronJobs',
  skills: 'nav.sub.skills',
  nodes: 'nav.sub.nodes',
  chat: 'nav.sub.chat',
  config: 'nav.sub.config',
  debug: 'nav.sub.debug',
  logs: 'nav.sub.logs',
};
...
export function renderApp(state: AppViewState) {
  ...
  const chatDisabledReason = state.connected ? null : t('topbar.disconnectedGateway');
    ...
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      ...
      <aside class="nav ${state.settings.navCollapsed ? "nav--collapsed" : ""}">
        ${TAB_GROUPS.map((group) => {
          ...
          const groupLabelKey = NAV_GROUP_KEYS[group.label];
          const groupLabel = groupLabelKey ? t(groupLabelKey) : group.label;
          return html`
        ...
        <div class="nav-group nav-group--links">
          ...
          <div class="nav-group__items">
            <a
              ...
              href="https://tom-byte-sys.github.io/OpenPollen/"
              target="_blank"
              rel="noreferrer"
      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        <section class="content-header">
          <div>
            ${state.tab === "usage" ? nothing : html`<div class="page-title">${t(titleKeyForTab(state.tab))}</div>`}
            ${state.tab === "usage" ? nothing : html`<div class="page-sub">${t(TAB_SUBTITLE_KEYS[state.tab] ?? 'nav.sub.overview')}</div>`}
          </div>
          <div class="page-meta">
