...
import { setLocale, onLocaleChange, detectBrowserLocale, type Locale } from "./i18n/index.ts";
...
export class OpenPollenApp extends LitElement {
  ...
  @state() sessionsDeleteConfirmKey: string | null = null;
  ...
  connectedCallback() {
    ...
    // Initialise i18n: sync module-level locale with the stored (or auto-detected) value.
    setLocale(this.locale);
    this.localeUnsubscribe = onLocaleChange((next) => {
      this.locale = next;
    });
  ...
  setAppLocale(next: Locale) {
    setLocale(next);
    this.applySettings({ ...this.settings, locale: next });
  }
